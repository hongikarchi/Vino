...
# v2 방향 전환: 문서에서 뽑은 체계 (2026-08-10 사용자 확정)

> v1(W1~W4)은 구현·라이브게이트 통과·push 완료. 이 절은 게이트 결과를 보고 사용자가 확정한
> **다음 방향**이며, v1을 폐기하지 않고 그 앞에 단계를 붙이는 전환이다.

## 왜 바꾸는가

라이브 게이트가 측정한 결정론적 매칭률은 **5/10**이었다. 놓친 것 중 둘(`콘크리트 벽`,
`외벽-콘크리트`)은 사람 눈에 명백히 벽인데, 매처가 레이어명 **전체**를 별칭과 비교하기 때문에
미스로 떨어졌다. 그러나 사용자가 지적한 더 근본적인 문제는 매칭률이 아니었다:

> "설계에 따라서, 디자이너에 따라서 그 형식이나 방식은 너무 달라지지 않나?"

즉 **WALL/COLUMN/FINISH라는 분류 어휘 자체를 우리가 정해서 들이민 것**이 문제다. 게이트에서
빈 `Default` 레이어와 `misc-stuff-01`이 CONCRETE로 분류된 것이 그 구조의 증상이다.
(그 둘은 게이트 스크립트가 기계적으로 첫 선택지를 고른 탓이라 제품 결함은 아니지만,
"모르면 뭐라도 붙인다"가 어떻게 보이는지는 정확히 보여준다.)

**전환**: 우리 사전이 권위이고 사용자 파일이 거기 맞춰지는 구조 → **사용자 파일이 원본이고,
거기서 뽑은 초안을 대화로 다듬어 확정한 것이 그 프로젝트의 체계**가 되는 구조.

## 구조상 핵심 두 가지

**1. "체계 정하기"와 "적용하기"를 분리한다.** 체계는 대화로 만드는 *문서*(프로젝트 테이블)이고,
적용은 이미 라이브 게이트를 통과한 승인 카드 흐름이다. 이렇게 나누면 검증된 적용 기계
(스냅샷 → CAS 배치 → 재관측)를 손대지 않고 그 **앞에** 단계 하나를 붙이는 것이 된다.

**2. 초안 만들기에서 결정론·판단·권한을 나눈다** (기존 원칙 그대로):

| 역할 | 담당 | 산출물 |
|---|---|---|
| 사실 | **서버 코드** | 이름 정규화·토큰화, 공유 토큰 빈도, 마크 패밀리 탐지(`SC1/SC2/SC5` → SC 계열) |
| 판단 | **모델** | 그 묶음에 이름·재료 **제안** |
| 권한 | **사용자** | 카드에서 확정·수정 |

## 확정된 결정 2건 (2026-08-10)

- **(a) 체계 저장 승인 = 카드 형식.** 대화 중 "응 해줘"로 쓰지 않는다. 체계가 틀리면 이후
  모든 매칭이 조용히 나빠지므로 구조화된 확인을 거친다. 기존 ApprovalCard가 이미
  행 단위 체크 + 선택지 라디오 + 근거 표시 + 서버가 채운 값을 갖고 있으므로,
  행이 *레이어*에서 *묶음*으로 바뀌는 정도의 확장으로 재사용한다.
- **(b) shipped 시드(`alias-seed-ko.json`) = 초안 힌트 전용.** 묶음 초안을 만들 때
  "이 12개에 '벽'이 있으니 WALL 계열 아닐까요?"라고 **제안하는 재료**로만 쓰고,
  사용자가 체계를 확정한 뒤에는 **매칭에 일절 관여하지 않는다**(폴백 아님).
  체계에 없는 이름이 나오면 우리 기본값으로 조용히 처리하지 않고 **묻는 것**이 정직한 동작이다.
  덤: 시드 자체의 품질 문제(별칭 `W`가 창호 부호와 충돌 등)는 판정하는 사전이 아니게 되므로
  대부분 무의미해진다.

## 단계

**P1. 이름 분석기 (읽기 전용, 쓰기 없음).** 레이어 이름을 정규화·토큰화해서 공유 토큰과
마크 패밀리로 묶고, 묶음별 구성원·빈도·근거를 보고한다. 모델도 사용자도 아직 아무것도 바꾸지 않는다.
- 먼저 하는 이유: 이것이 곧 **실무 파일 실측**이다. 산출물이 바로 "이 파일은 이렇게 생겼다"는
  데이터이고, 나머지 단계 전체의 입력이며, 아래 기술 리스크의 판정 수단이다.
- 검증: 합성 이름 목록 단위 테스트 + 실무 파일(33MB, 70레이어) 1회 실행.

**P2. 체계 저장.** 확정된 묶음을 프로젝트 테이블에 쓰는 typed op(스키마 검증 포함).
매처는 **프로젝트 체계를 먼저** 보고, 체계가 없을 때만 현행 동작으로 떨어진다 —
기존 동작이 깨지지 않는 점진 전환.

**P3. 대화 루프 + 체계 카드.** "레이어 70개를 읽었습니다. 이렇게 묶이는 것 같습니다 […].
맞나요?" → 사용자가 말로 수정 → 수렴 → **체계 카드로 확정**(결정 a). 하우스룰에 이 흐름을
명시하고 P1·P2 도구를 물린다.

**P4. 적용.** 기존 카드 흐름 그대로. 다만 제안의 근거가 *우리 시드*가 아니라
*사용자가 확정한 체계*다.

## 이 전환이 흡수하는 기존 미결 판단 3건

- **복합어 매칭**: 체계의 별칭이 곧 그 파일의 실제 이름이므로 구조적으로 해결. 부분 일치
  계층은 이제 필수가 아니라 *변형 대응용 선택지*로 격하(`외벽-콘크리트-A` 같은 파생형).
- **빈 레이어**: 초안 도출에서 제외(empty leaf 판정은 기존 `layerIntegrity` 어휘 재사용).
- **어휘 축적**: 부가 기능이 아니라 **주 메커니즘**이 된다.

## 기술 리스크 (정직하게)

**한국어 복합어는 토큰 분리가 되지 않는다.** `외벽`은 공백이 없어 토큰으로 쪼개도 `외벽`
하나이고, 안에서 `벽`을 찾으려면 부분 문자열/n-gram이 필요하다. 즉 **P1 군집화 품질이 이
기능 전체의 성패**이고, 영어보다 확실히 어렵다. 설계로 미리 장담할 수 없으며 **P1을 실무
파일에 돌린 결과가 판정한다.** 결과가 부족하면 군집 기준을 이름 외 신호로 넓히는 대안:
레이어 계층 구조(부모-자식), 현재 색, 레이어 위 오브젝트 타입 분포.

## 아직 정하지 않은 것

- 체계의 축: 요소(WALL)와 재료(concrete)가 지금은 한 항목에 묶여 있다. `목재 벽`이 들어오면
  WALL/concrete로 잡혀 재료가 틀린다. 두 축 분리 여부는 P1 실측 결과를 보고 판단.
- 색과 라벨의 분리 승인(색만/라벨만) 여부 — v1에서 보류된 항목, 여전히 열려 있음.

## v2 진행 상태 (2026-08-10 마감 시점)

**P1~P4 전부 구현·실무모델 라이브 검증 완료.** 커밋: `6c3fe75`(P1) `5a22c7d`(P1 보강)
`44fd07c`(축 분리) `870c794`(P3) `46bddbe`(후속 2건). 전부 push + 패키지 재설치됨.

| 단계 | 상태 | 검증 |
|---|---|---|
| P1 이름 분석기 | 완료 | 실무 70레이어에서 **커버리지 94%**(미분류 4). 모델이 도구 설명만으로 자율 호출 |
| P2 체계 저장 (2축) | 완료 | `철골::철골_3D::SC5 (Bracing)` → 요소=기둥·재료=steel 단위 테스트로 고정 |
| P3 대화+체계 카드 | 완료 | 실무모델 종단: 요청 1회 → 2축 카드 4행 → 승인 → 체계 저장 → 다음 턴 제안이 그 체계로 해석 |
| P4 적용 | 완료(v1 그대로) | v1 라이브 게이트 PASS 시점의 적용 기계를 손대지 않음 |

**v2가 바꾼 결과**: 시드가 SC를 concrete로 단정하던 것이 사라지고, 사용자 파일의 계층
(부모=재료, 말단 부호=요소)에서 뽑은 체계가 권위가 됐다. 철골 아래 32개 레이어가 전부
material=steel로, SC1~7이 사용자 어휘 "기둥"으로 해석된다.

### 후속 처리 (실무모델 실측 후)

- **일괄 승인 복구**: 색이 있는 행을 전부 체크 해제하던 동작이 실무 파일에서 32행 전부
  해제로 이어져 일괄 승인을 무의미하게 만들었다(없애려던 수작업). 해석된 행은 체크된 채
  오고, 기존 색은 표시(`customColour`)로만. 진짜 레버는 카드 레벨 `colorPolicy`
  (재료 색으로 칠하기 / 기존 색 유지=라벨만) — 행 체크는 라벨과 색을 함께 덮으므로
  "라벨만"은 행 단위로 표현 불가였다.
- **빈 leaf 제외**: Default가 concrete로 분류된 원인. 빈 **parent**는 유지(철골이 하위
  재료를 선언하는 스코프이므로).

### 남은 것

1. **회귀 게이트 재실행 (유일한 실질 공백)** — `gate-layer-curation.ps1`은 v1 흐름을 기준으로
   쓰였고, 그 뒤 빈-leaf 제외와 preChecked 의미가 바뀌었다. 통과할 가능성이 높지만
   **확인되지 않았다.** Rhino가 비는 시점에 1회 실행 필요.
2. 의도적 이연: IFC 매핑, 물리 rename(영향 분석 선행), 개인 계층 승격, Data 탭 라벨 뷰.
3. 관측: `build-package` 전체 실행이 1회 exit 255 후 재실행 통과 — AgentHost.Tests 간헐
   실패 가능성, 재현 시 조사.
