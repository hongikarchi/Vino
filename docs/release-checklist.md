...
## 0-1. 2026-08-14 갱신 — 리네임 실행됨

제품명이 **Vino**(Vibe Rhino)로 확정되어 RhinoVibe 계획을 대체했고, 리네임 커밋이 이 브랜치(`vino-rename`)에 랜딩했습니다. 이 문서의 아래 절들은 그 이전에 작성된 것이므로 다음 상태 변화를 먼저 반영해서 읽을 것:

- **§1-① 이름 결정: 완료.** Yak `vino` 미선점 확인(404) 후 확정. manifest `name: Vino`.
- **§1-② 문서 임베드 키: "유지" 결정.** `GPTino.SourceDocKey`·`GPTino.LogicalEntityId`·`gptino_bake_*`·`gptino.` 레이어 키·`gptino:auto/absent/default/guard` sentinel은 사용자 문서·DB·모델 wire에 새겨진 값이라 **레거시 wire 상수로 보존**(기존 파일의 provenance·재bake 정체성 유지). 예외: `GPTino::Quarantine` 레이어명(사용자 가시)과 `gptino_v1` 툴 네임스페이스는 Vino로 변경, 디스패처가 구 네임스페이스를 한시 수용.
- **§2 리네임: 완료.** 파이프 `vino-*`·HMAC 도메인·`VINO_*` env(+스크러빙 필터 3곳)·`vino://`·명령 `VinoOpenPanel`·`.vino-instance.lock` 원자 변경, `BridgeProtocol.Version` **20→21** bump(문서의 "현재 19" 표기는 낡은 것이었음).
- **§2-3 이관: 완료.** 데이터루트 `%LOCALAPPDATA%\Vino`. 구 GPTino 루트는 `LegacyDataDirectoryAdoption`이 프로젝트 첫 오픈 시 1회 복사(구 락 이름도 라이브 감지), 아카이브 브라우저는 구 부모를 읽기 전용 폴백으로 나열. 구 `backups\`는 이관하지 않음(사고 복구 필요 시 구 경로 수동 참조).
- **§3-5 막다른 오류 메시지 2개: 수정.** 실제 예외 메시지/stderr 내용을 표시하고 VINO_DEV_MODE 안내로 교체.
- **§4 "GPT" 문구: 수정.** "Sign in with ChatGPT" 계열로 교체, 브랜드 마크 G→V.
- **GitHub 리포명: 완료(08-14).** `hongikarchi/GPTino` → `hongikarchi/Vino` rename 실행(사용자 승인), 저장소 URL 5곳 일괄 교체, 구 URL은 GitHub 리다이렉트로 유지됨.
- **작은 로고: 완료.** 와인잔 아이콘 7종 + `vino-logo.svg` 커밋(`c53603e`). 큰 로고(코뿔소)는 시안 단계에서 사용자 인수.
- **미해결로 남긴 것:** 매니페스트 메타데이터(§3-3), 서명·태그·깨끗한 머신 설치 검증, 지원 이메일.
...
| # | 결정 | 지금 상태 | 미루면 |
|---|---|---|---|
| 1 | **Yak 패키지 이름을 `RhinoVibe`로 바꾼다** | `manifest.yml:2` = `Vino` | Yak은 개명을 새 패키지로 취급합니다. 이미 배포한 뒤 바꾸면 기존 사용자에게 **두 패키지가 동시 설치**되고, 두 `.rhp`가 **같은 플러그인 GUID**로 한 Rhino에 로드되며, 두 AgentHost가 같은 데이터 루트를 다툽니다. 아직 아무도 설치하지 않았으므로 지금은 비용 0 |
| 2 | **문서에 새겨지는 키를 바꿀 것인가** | `GPTino.LogicalEntityId`, `GPTino.SourceDocKey`, `gptino_bake_family`, `gptino.` 레이어 네임스페이스, `Vino::Quarantine` 레이어 | 이 키들은 **사용자의 .3dm 안에 저장**됩니다. 출시 후 변경 = 기존 파일의 객체 출처 표식 상실 → 에이전트가 자기 과거 산출물을 "사용자 소유 기하"로 보고 승인을 요구하기 시작. 지금은 본인 개발용 파일만 영향 |
| 3 | **Rhino 명령 `VinoOpenPanel`** | `VinoOpenPanelCommand.cs:9` | 사용자가 직접 타이핑하고 툴바 버튼·별칭에 넣는 이름. 개명 시 구 이름 명령을 얇게 하나 남겨 전달하는 방식을 권장 |
...
| 대상 | 위치 |
|---|---|
| 플러그인 GUID `b903e20d-1cb3-4d8e-b37d-9be263a678d4` | `VinoPlugIn.cs:8` + `Properties/AssemblyInfo.cs:4` (**두 곳 동기 필수**), 검증 `PluginMetadataTests.cs:9` |
| 패널 GUID `91ab786f-…` | `VinoPanel.cs:9` |
| GH 어셈블리 GUID `d2b0c9b2-…` | `VinoAssemblyInfo.cs:27` |
...
- **네임드 파이프**: `NamedPipeTransport.cs:29` 생성 + `:36` `StartsWith("vino-")` 검증자 + `:41` 오류문구
- **HMAC 도메인 분리 문자열**: `BridgeAuthentication.cs:40,41` — 어긋나면 핸드셰이크가 전부 인증 거부(원인 안 보임)
- **`VINO_*` 환경변수** (`BRIDGE_PIPE`, `BRIDGE_SECRET`, `API_TOKEN`, `READY`, `PYTHON`, `DEV_MODE`, `DEV_DATA_DIRECTORY`) — `.rhp` → `AgentHost.exe` → `Terminal.exe` 3단 계약
- ⚠️ **위 변수를 바꾸면 스크러빙 필터도 같이 바꿔야 합니다.** `AgentHostBootstrapper.cs:349,350`, `CodexAppServerClient.cs:1262,1263`, `TerminalLauncher.cs:174,175`가 `StartsWith("VINO_")`로 **Codex 자식 프로세스에 넘기기 전 비밀을 제거**합니다. 변수만 바꾸고 필터를 놔두면 **비밀이 모델의 자식 프로세스로 새어 나갑니다.** 로그 마스킹 정규식(`DevLoop/Program.cs:1209`)도 동일
- **HTTP 인증 표면**: 헤더 `X-Vino-Token`·`X-Vino-Panel-Parent`, 쿠키 `vino_runtime`
- **기동 핸드셰이크 토큰** `VINO_READY ` — 안 맞으면 플러그인이 준비 신호를 영원히 기다리다 타임아웃
- **패널 URI 스킴 `vino://`** — C# 인터셉터(`VinoPanel.cs:15`)와 React CTA(`App.tsx:70`, `NoGrasshopper.tsx`) 두 코드베이스가 동시에 발행

### 2-3. 이름 변경이 아니라 "이관"이 필요한 것

- `%LOCALAPPDATA%\Vino\projects\<fingerprint>` — **두 곳에 독립 정의**(`AgentHostOptions.cs:75`, `ProjectArchiveReader.cs:42`). 어긋나면 아카이브 브라우저가 빈 목록
- `%LOCALAPPDATA%\Vino\backups` (`VinoDocumentBackup.cs:26,29`) — 사고 복구용 백업. 고아가 되면 사용자가 정확히 필요할 때 찾지 못함
- 🔴 **`.vino-instance.lock`** (`RuntimeInstanceLock.cs:11`) — 구/신 빌드가 서로 다른 락 이름을 쓰면 **둘 다 같은 데이터 루트를 점유**해 SQLite 동시 쓰기 → DB 손상. 이 목록에서 유일하게 데이터를 망가뜨릴 수 있는 항목
...
- **`vino_v1` 툴 네임스페이스** (`DynamicToolSpecs.cs:63`) — 모델이 호출하는 이름. 스키마를 어차피 바꾸는 게 아니라면 유지가 안전합니다. 진행 중이던 Codex 스레드가 재개되면 옛 이름으로 계속 호출합니다
...
1. 🟢 **`.yak` 빌드 검증 완료 (2026-08-10)** — GitHub에서 깨끗하게 클론(`5620cef`)해 `build-package.ps1 -BuildYak`를 끝까지 돌렸고 성공했습니다. .NET 테스트 738건 + vitest 59건 전부 통과(의도적 Skip 1건은 live-codex), 산출물 `vino-0.1.0-alpha.7-rh8_21-win.yak` **74.9MB**. 남은 절반은 **깨끗한 Rhino 8.21에 설치해보는 것** — 이 머신에는 이미 패키지가 설치돼 있어 여기서는 검증할 수 없습니다
...
3. 🔴 **매니페스트 내용이 자리표시자 수준** — `authors: Vino contributors`(익명), description이 **Codex CLI + OpenAI 계정이 필수라는 사실을 한 마디도 안 함**, 키워드 빈약. `AssemblyInfo.cs:7,9`의 Email/Phone이 **빈 문자열**이라 Rhino 플러그인 상세에 연락처가 공란
4. 🔴 **git 태그가 0개**인데 `SECURITY.md:27`은 "최신 태그 프리릴리스를 지원한다"고 약속
5. 🔴 **막다른 오류 메시지 2개** — `VinoPlugIn.cs:68`("bounded development diagnostics를 보라")과 `AgentHostBootstrapper.cs:465`("local runtime log를 보라")가 **릴리스 설치본에 존재하지 않는 것**을 가리킵니다. 진단 추적은 `VINO_DEV_MODE=1`에서만 동작. 로드 실패한 사용자는 아무것도 없는 곳을 보게 됩니다
...
- 🔴 **패널 UI가 "GPT"를 브랜드처럼 씁니다** — `App.tsx:290` "Sign in to GPT", `:303` "Log in to GPT", `:293` "Vino drives GPT through the Codex CLI". 제품명에서 GPT를 빼는 목적이 여기서 무너집니다. "OpenAI Codex"로 교체 권장
- `client.ts:199` 하드코딩 기본 모델 `gpt-5.6-sol`, `mock.ts:188-205` 데모 카탈로그의 `GPT-5.6 Sol/Terra/Luna` 표기
- **GitHub 리포명** `Vino` → `RhinoVibe` (public, 스타 1·포크 0 — GitHub가 옛 URL 리다이렉트를 유지하므로 비용 낮음). `manifest.yml:9`, `AssemblyInfo.cs:5,10,11`, `VinoAssemblyInfo.cs:31`, `Directory.Build.props:12`가 전부 이 URL
...
- `ui/panel/src/api/mock.ts:522`에 개발자 절대경로 `C:\Users\user\AppData\Local\Vino\...`가 번들에 포함
...
시작점은 `Vino.History.Tests`에서 LibGit2Sharp가 내는 `path too long`입니다:
...
& "C:\Program Files\Rhino 8\System\Yak.exe" install --source "C:\받은폴더" Vino 0.1.0-alpha.7
# 그다음 Rhino 재시작 → 패널 열기
```
...
- 세션 데이터는 테스터의 `%LOCALAPPDATA%\Vino`에 쌓이고, 언인스톨해도 자동 삭제되지 않습니다
...
2. **리네임을 한 커밋으로** — 원자 그룹(§2-2) 동시 변경, 데이터 이관(§2-3) 포함, `BridgeProtocol.Version`을 현재 값(19)에서 한 단계 bump, `vino_v1`은 유지
