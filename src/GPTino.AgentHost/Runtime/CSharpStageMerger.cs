...
internal static class CSharpStageMerger
{
    ...
    internal const string MetaMarkerPrefix = "// <gptino:stages v1> ";
    ...
    internal static bool TryParseLayout(string source, out MergedLayout? layout, out string? error)
    {
        ...
        foreach (var rawLine in lines)
        {
            ...
            if (trimmed.StartsWith(MetaMarkerPrefix, StringComparison.Ordinal))
            {
                ...
                {
                    error = $"The gptino:stages meta header does not parse: {exception.Message}";
                    return false;
                }
                continue;
            }
        ...
        {
            error = "No gptino:stages v1 meta header found - this is not a merged component.";
            return false;
        }
    ...
    private static ParsedStage ParseStage(StageSpec stage)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Stage '{stage.NickName}' ({stage.BlockId}) already contains gptino stage/seam markers - " +
