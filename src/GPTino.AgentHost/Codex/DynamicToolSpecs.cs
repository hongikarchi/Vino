...
internal static class DynamicToolSpecs
{
    ...
    internal const string DefaultPayloadGuide = """
        ...
        - acceptancePredicates may be [] — the server attaches the standard set (creates/bakes objectExists, deletes objectAbsent, wires wireExists/wireAbsent, everything else runtimeErrorAbsent). BUT a change meant to PRODUCE a result (compute/generate geometry or values that flow out of a component) MUST add outputCountInRange '<outputName>:1:*' on the terminal output — objectExists/runtimeErrorAbsent do NOT catch an empty output, so a producing change whose wiring/typing yielded nothing would else commit as a false success. Skip only for pure scaffolding (an unwired component) or non-output changes (move/rename/group/delete).
