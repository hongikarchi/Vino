...
public sealed class CodexAppServerClient : ICodexSessionClient, IModelCatalog, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new Dictionary<string, object?>
        {
            ...
            // operator's ~/.codex/config.toml service_tier, so Vino authoring is fast on every
    ...
    private static string CreateDisabledMcpTableOverride(IReadOnlyCollection<string> names)
    {
        ...
        foreach (var name in names)
        {
            ...
            result.Append("\"={enabled=false,command=\"vino-disabled\"}");
        }
        result.Append('}');
    ...
    private async Task<ProcessGeneration> StartCandidateAsync(string executable, CancellationToken cancellationToken)
    {
        ...
        _ = await CallCoreAsync(
            ...
            new
            {
                clientInfo = new { name = "vino-agent-host", title = "Vino", version = "0.1.0" },
    ...
    private static Dictionary<string, object?> CreateThreadStartParameters(string cwd, string? model)
    {
        var parameters = new Dictionary<string, object?>
        {
            ...
            // (own threads, parentThreadId) and are not Vino sessions, so they cannot call
            // the session-bound vino_v1 tools — they draft, the parent session submits.
            ["multiAgentMode"] = "proactive",
    ...
    private const string ThreadInstructions = """
        ...
        Codex sub-agents cannot call the session-bound vino_v1 tools: use them only to draft code or payload text,
