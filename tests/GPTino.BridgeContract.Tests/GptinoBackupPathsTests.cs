...
public sealed class GptinoBackupPathsTests
{
    [Fact]
    public void PruneKeepsTheMostRecentFoldersAndDeletesTheRest()
    {
        ...
        var removed = GptinoBackupPaths.PruneToMostRecent(temp.Path, keep: 2);
    ...
    public void PruneIsANoOpWhenWithinTheLimitOrTheRootIsMissing()
    {
        ...
        Assert.Equal(0, GptinoBackupPaths.PruneToMostRecent(temp.Path, keep: 6));
        Assert.True(Directory.Exists(Path.Combine(temp.Path, "only")));

        Assert.Equal(0, GptinoBackupPaths.PruneToMostRecent(Path.Combine(temp.Path, "does-not-exist"), keep: 3));
    }
    ...
    private sealed class TempDir : IDisposable
    {
        public string Path { get; } = System.IO.Path.Combine(
            ...
            "gptino-backup-prune-" + Guid.NewGuid().ToString("N"));
