...
public sealed class SessionOrchestratorTests
{
    ...
    /// vino.* label keys and the ARGB int — so nothing is re-derived or invented model-side.
    ...
    public async Task TurnInputCarriesSelectionContextHintWithoutAlteringTheStoredMessage()
    {
        ...
        await WaitForStateAsync(harness.Store, harness.Session.Id, SessionStates.Idle);
        var startedTurn = Assert.Single(client.StartedTurns);
        Assert.StartsWith("<vino_context>", startedTurn.Message, StringComparison.Ordinal);
    ...
    public async Task TurnInputCarriesGrasshopperSelectionContextHint()
    {
        ...
        var selectionContext = new StaticSelectionContext(new Vino.BridgeContract.SelectionChangedEvent(
            ...
            DateTimeOffset.UtcNow,
            [
                new Vino.BridgeContract.GrasshopperSelectedObject(
    ...
    public async Task ImageAttachmentsAreDeliveredAsImageInputsAndKeptOutOfTheTextBlock()
    {
        ...
        Assert.Contains("<vino_attachments>", startedTurn.Message, StringComparison.Ordinal);
        Assert.Contains("notes.txt", startedTurn.Message, StringComparison.Ordinal);
    }

    private sealed class StaticSelectionContext(Vino.BridgeContract.SelectionChangedEvent? selection)
        : ISelectionContextSource
    {
        public Vino.BridgeContract.SelectionChangedEvent? CurrentSelection { get; } = selection;
    ...
    public async Task TurnSelectionContextFollowsTheSessionsGrasshopperDocBinding()
    {
        ...
        var defaultSelection = new Vino.BridgeContract.SelectionChangedEvent(
            [],
            null,
            DateTimeOffset.UtcNow,
            [
    ...
    private sealed class RoutingSelectionContext(
        string boundDocKey,
        Vino.BridgeContract.SelectionChangedEvent boundSelection,
        Vino.BridgeContract.SelectionChangedEvent defaultSelection)
        : ISelectionContextSource
    {
        public Vino.BridgeContract.SelectionChangedEvent? CurrentSelection => defaultSelection;

        public CanvasDigest? CurrentCanvasDigest => null;

        public Vino.BridgeContract.SelectionChangedEvent? SelectionFor(string? docKey) =>
    ...
    public async Task AttachmentsPersistShortLinesAndCarryPathsOnlyInTheTurnInput()
    {
        ...
        await WaitForStateAsync(harness.Store, harness.Session.Id, SessionStates.Idle);
        var startedTurn = Assert.Single(client.StartedTurns);
        Assert.StartsWith("read the attached note\n<vino_attachments>", startedTurn.Message, StringComparison.Ordinal);
        Assert.Contains("note.txt (text/plain): ", startedTurn.Message, StringComparison.Ordinal);
        Assert.EndsWith("</vino_attachments>", startedTurn.Message, StringComparison.Ordinal);
    ...
    public async Task CompletionNotificationStillPerformsFinalReadAndDeduplicatesItsItem()
    {
        ...
        Assert.Equal($"vino-codex-v1:turn-1:final_answer:{hash}", persistedId);
    }
